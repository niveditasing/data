...
def download_and_segregate_by_gas():
    ...
    try:
        ...
        logging.info(f"Successfully fetched {len(api_country_codes)} countries from API ({countries_url}).")
    except requests.exceptions.RequestException as e:
        status_code = e.response.status_code if e.response is not None else "N/A"
        response_body = e.response.text if e.response is not None else "No response body"
        logging.error(
            f"Error fetching country list from API ({countries_url}) - Status: {status_code}, Response: {response_body}: {e}"
        )
    ...
    for gas in gases:
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            future_to_iso = {
                executor.submit(download_and_process_zip, f"{base_url}/{gas}/{iso}.zip", iso, gas): iso
            ...
            for future in as_completed(future_to_iso):
                ...
                try:
                    ...
                    logging.error(f"Download failed for {iso} ({gas}): {e}")
                    raise
